      ...
      case 'missing-client-identifier':
        if (kIsWeb) {
          ...
          return 'Web reCAPTCHA/app-credential check rejected by Google\'s '
              ...
              'If the browser console also logs "Failed to initialize '
              'reCAPTCHA Enterprise config. Triggering the reCAPTCHA v2 '
              'verification." -- that line itself is a benign, automatic '
              'SDK fallback, NOT the bug (Enterprise domain settings you '
              'configured no longer apply once it has fallen back to v2). '
              'The Enterprise config fetch failing usually means either '
              '"reCAPTCHA Enterprise API" is not ENABLED under APIs & '
              'Services -> Library for this project (a key can exist under '
              'Security -> reCAPTCHA Enterprise while the API itself is '
              'off/restricted), or a browser ad-blocker/privacy extension '
              'is blocking Google\'s recaptcha script -- retest in an '
              'Incognito/private window with all extensions disabled '
              'before assuming it is still a project config issue. '
              'Also allow a few minutes for a just-changed Authorized '
              'domains / reCAPTCHA setting to propagate. '
