...
CollegeModel _college({
  ...
  return CollegeModel(
    ...
    placements: const CollegePlacements(highestPackageLpa: 12, averagePackageLpa: 6, placementPercentage: 80),
    aggregatedRatings: const CollegeRatings(overall: 4, faculty: 4, infrastructure: 4, placements: 4, campusLife: 4, hostel: 4, teaching: 4),
  );
}
...
void main() {
  ...
  group('audit targets documented', () {
    ...
    test('exhaust batch has no artificial tiny page cap', () {
      expect(CollegeConstants.searchExhaustBatchSize, greaterThanOrEqualTo(500));
    });
  });
  ...
  group('resolveSearchIntent searchable fields', () {
    ...
    test('ownership maps to type', () {
      expect(CollegeSearchUtils.resolveSearchIntent(ownership: 'Government').type, 'government');
      expect(CollegeSearchUtils.resolveSearchIntent(ownership: 'private').type, 'private');
    });

    test('university filter preserved', () {
      final i = CollegeSearchUtils.resolveSearchIntent(university: 'Savitribai Phule Pune University');
      expect(i.university, 'Savitribai Phule Pune University');
    });

    test('category city state combination', () {
      final i = CollegeSearchUtils.resolveSearchIntent(category: 'Engineering', city: 'Pune', state: 'Maharashtra');
  ...
  group('applyFilters ownership university combined', () {
    final sample = [
      _college(id: '1', name: 'Alpha Engineering', city: 'Pune', state: 'Maharashtra', category: 'Engineering', type: 'government', universityName: 'SPPU'),
      _college(id: '2', name: 'Beta Medical', city: 'Mumbai', state: 'Maharashtra', category: 'Medical', type: 'private', searchTokens: const ['aiims', 'delhi']),
      _college(id: '3', name: 'Gamma Nursing', city: 'Pune', state: 'Maharashtra', category: 'Nursing', type: 'private'),
    ];

    test('city Pune returns every Pune college', () {
      expect(CollegeSearchUtils.applyFilters(sample, city: 'Pune').map((c) => c.id), ['1', '3']);
    });

    test('category Engineering', () {
      expect(CollegeSearchUtils.applyFilters(sample, category: 'Engineering').map((c) => c.id), ['1']);
    });

    test('ownership type government', () {
      expect(CollegeSearchUtils.applyFilters(sample, type: 'government').map((c) => c.id), ['1']);
    });

    test('university matches name field', () {
      expect(CollegeSearchUtils.applyFilters(sample, university: 'SPPU').map((c) => c.id), ['1']);
    });

    test('university falls back to searchTokens', () {
      expect(CollegeSearchUtils.applyFilters(sample, university: 'aiims').map((c) => c.id), ['2']);
    });

    test('combined category city state', () {
      expect(CollegeSearchUtils.applyFilters(sample, category: 'Engineering', city: 'Pune', state: 'Maharashtra').map((c) => c.id), ['1']);
    });

    test('cityLower used when city display empty', () {
      final odd = _college(id: '4', name: 'Delta', city: '', state: 'Maharashtra', category: 'Engineering', cityLower: 'pune');
      expect(CollegeSearchUtils.applyFilters([odd], city: 'Pune').map((c) => c.id), ['4']);
    });
  });

  group('session cache clear', () {
    test('clearSearch empties cached search results', () {
      CollegeSessionCache.setSearch([_college(id: 'x', name: 'X', city: 'Pune', state: 'Maharashtra', category: 'Engineering')]);
  ...
  group('bundled offline incomplete vs audit', () {
    test('bundled dataset must not be treated as full production', () async {
      ...
      final pune = all.where((c) => CollegeSearchUtils.normalizeCity(c.city) == 'pune').length;
